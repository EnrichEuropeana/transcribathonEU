...
$.widget( "ui.iviewer", $.ui.mouse, {
    ...
    **/
	...
	// This function was added to enable free dragging
	setLooseCoords: function(x,y)
    {
        //do nothing while image is being loaded
        if(!this.img_object.loaded()) { return; }

        this.img_object.x(parseInt(x, 10));
        this.img_object.y(parseInt(y, 10));
    },
    ...
    **/
    _mouseDrag: function(e)
    {
		...
		//this.setCoords(lleft, ltop); // ORIGINAL
		this.setLooseCoords(lleft, ltop); // OLAF
    ...
    _dblclick: function(ev)
    {
      ...
      if (this.options.zoom_on_dblclick) {
        var container_offset = this.container.offset()
		  ...
		  getFullIfNotYet(document.getElementById('fsgo'));
    ...
    **/
    createui: function()
    {
		...
		$("<div>", { 'class': "iviewer_editor_toggle iviewer_common iviewer_button",'id':'fs_editor_toggle' })
                    .bind('mousedown touchstart',function(){toggleFSEditor('fs_editor_toggle'); return false;})
					.appendTo(this.container);
		
		if(this.container.hasClass('fullscreen')){	
		obj = this.container.parent().attr('id');		
		$("<div>", { 'class': "iviewer_fullscreen_toggle iviewer_common iviewer_button" })
                    .bind('mousedown touchstart',function(){toggleFull(document.getElementById(obj)); /*me.fit(this);*/ return false;})
					.appendTo(this.container);
		}
		//$('p.transcribe-but.act').bind('mousedown touchstart',function(){toggleFull(document.getElementById(obj)); return false;});
		/*$("<div>", { 'class': "iviewer_flip_horizontal iviewer_common iviewer_button" })
                    .bind('mousedown touchstart',function(){me.flip_h(); return false;})
                    .appendTo(this.container);*/
