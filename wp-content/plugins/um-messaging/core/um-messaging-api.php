...
class UM_Messaging_Main_API {
	...
	***/
	function can_message( $recipient ) {
		...
		// only friends
		if ( $who_can_pm == 'friends' && defined('um_friends_extension') ) {
			global $um_friends;
			if ( !$um_friends->api->is_friend( get_current_user_id(), $recipient ) ) {
				return false;
			}
		}
	...
	***/
	function get_unread_count( $user_id ) {
		...
		if(count($blocked)) {
			$count = $wpdb->get_var("SELECT COUNT(message_id) FROM {$this->table_name2} WHERE recipient={$user_id} AND author NOT IN(" . implode(',', $blocked) . ") AND status=0 LIMIT 10");
		} else {
	...
	***/
	function create_conversation( $user1, $user2 ) {
		...
		$conversation_id = $wpdb->get_var(
			$wpdb->prepare("SELECT conversation_id FROM {$this->table_name1} WHERE user_a = %d AND user_b = %d LIMIT 1",
				$user1,
				$user2
			)
		);
	...
	***/
	...
	public function set_redirect_to( $url ){
		...
		$url = add_query_arg('profiletab','messages', $url);
		return $url;
	}
}
