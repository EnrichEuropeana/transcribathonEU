	...
	***/
	...
	function um_messaging_update(){
		...
		$results = $wpdb->get_results(
			$wpdb->prepare("SELECT last_updated FROM {$table_name1} WHERE conversation_id = %d LIMIT 1",$conversation_id)
		);
		...
		if ( $results[0]->last_updated > $_POST['last_updated'] ) {
			...
			$messages = $wpdb->get_results(
				$wpdb->prepare("SELECT * FROM {$table_name2} WHERE conversation_id = %d AND time > %s ORDER BY time ASC LIMIT 1",
					$conversation_id,
					$last_updated
				)
			);
