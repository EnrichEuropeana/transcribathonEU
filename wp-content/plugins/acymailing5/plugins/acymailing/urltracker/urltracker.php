...
class plgAcymailingUrltracker
{

    function __construct(&$subject, $config)
    {
        if (!isset($this->params)) {
            $plugin = acymailing_getPlugin('acymailing', 'urltracker');
        ...
        foreach ($results[1] as $i => $url) {
            ...
            if ($internalUrl) {
                ...
                if (strpos($urlWithoutBase, '/') || strpos($urlWithoutBase, '?')) {
                    ...
                    $path = str_replace('/', DS, $urlWithoutBase);
                    if(substr($path, strrpos($path, '.')) != '.php' && @file_exists(ACYMAILING_ROOT.DS.$path)){
                        $isFile = true;
                    }
                }
            }
