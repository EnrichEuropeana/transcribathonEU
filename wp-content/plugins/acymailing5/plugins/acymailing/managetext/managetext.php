...
class plgAcymailingManagetext{
	var $foundtags = array();

	function __construct(&$subject, $config){
		if(!isset($this->params)){
			$plugin = acymailing_getPlugin('acymailing', 'managetext');
	...
	function acymailing_replacetags(&$email, $send = true){
		...
		if(strpos($email->body, '</body>')) $email->body = str_replace('</body>', $imageAlignment.'</body>', $email->body);
		else $email->body .= $imageAlignment;
	}
	...
	private function _replaceConstant(&$email){
		...
		foreach($tags as $i => $oneTag){
			...
			if($type == 'const'){
				...
				if($val == 'sitename'){
					$tagsReplaced[$i] = acymailing_getCMSConfig($val);
				}
			}else{
				static $done = false;
				if(!$done && strpos($val, 'COM_USERS') !== false){
					...
					acymailing_loadLanguageFile('com_users');
				}
				if(!empty($arrayVal)){
