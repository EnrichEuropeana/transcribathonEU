...
class plgAcymailingStats{
	function __construct(&$subject, $config){
		if(!isset($this->params)){
			$plugin = acymailing_getPlugin('acymailing', 'stats');
	...
	function statPicture(&$email, $send = true){
		...
		if(!$send){
			$pictureLink = ACYMAILING_LIVE.$this->params->get('picture', ACYMAILING_MEDIA_FOLDER.'/images/statpicture.png');
		}else {
			$config = acymailing_config();
			$itemId = $config->get('itemid', 0);
			$item = empty($itemId) ? '' : '&Itemid=' . $itemId;
			$pictureLink = acymailing_rootURI().acymailing_addPageParam('statistics&mailid=' . $email->mailid . '&subid={subtag:subid}' . $item, true, true);
		}
	...
	function acymailing_getstatpicture(){
		return $this->params->get('picture', ACYMAILING_MEDIA_FOLDER.'/images/statpicture.png');
	}
	...
	function acymailing_stats_show(){
		...
		$query = 'SELECT SQL_CALC_FOUND_ROWS a.*, GROUP_CONCAT(c.name SEPARATOR ", ") AS listname, u.'.$this->cmsUserVars->name.' AS username
					...
					JOIN '.acymailing_table($this->cmsUserVars->table, false).' AS u ON a.userid = u.'.$this->cmsUserVars->id.' ';

		$searchFields = array('a.mailid', 'a.subject', 'u.'.$this->cmsUserVars->name);
	...
	function _chartImages($tag){
		...
		if(!empty($tag->sent)){
			...
			if(file_exists(ACYMAILING_MEDIA.'statistic_charts'.DS.$filename)) {
				$oneChart->chart = '<img src="' . ACYMAILING_LIVE . ACYMAILING_MEDIA_FOLDER.'/statistic_charts/' . $filename . '"/>';
			}else{
				$created = piechartToImage($filename, 200, 200, array_values($values), $colors);
				if($created) $oneChart->chart = '<img src="' . ACYMAILING_LIVE . ACYMAILING_MEDIA_FOLDER.'/statistic_charts/' . $filename . '"/>';
			}
