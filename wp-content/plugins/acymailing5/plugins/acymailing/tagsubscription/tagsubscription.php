...
class plgAcymailingTagsubscription
{
    ...
    function __construct(&$subject, $config)
    {
        if (!isset($this->params)) {
            $plugin = acymailing_getPlugin('acymailing', 'tagsubscription');
    ...
    function acymailingtagsubscription_show()
    {
        ...
        $text = '<div id="iframelists" style="display:none;"><iframe src="'.acymailing_completeLink('chooselist&popup=0&task=listids&all=0', true).'" width="98%" height="100%" scrolling="auto"></iframe></div>
    ...
    function acymailing_replaceusertags(&$email, &$user, $send = true)
    {
        ...
        if ($this->unsubscribeLink && !$this->listunsubscribe && $this->params->get('listunsubscribe', 0) && method_exists($email, 'addCustomHeader')) {
            ...
            $myLink = acymailing_frontendLink('user&subid='.intval($user->subid).'&task=unsub&mailid='.$email->mailid.'&key='.urlencode($user->key).$lang.'&'.acymailing_noTemplate());
    ...
    private function _listowner(&$email, &$user, &$parameter)
    {
        ...
        if (!isset($this->listsowner[$listid])) {
            $this->listsowner[$listid] = acymailing_loadObject('SELECT u.* FROM #__acymailing_list as list JOIN '.acymailing_table($this->cmsUserVars->table, false).' as u ON u.'.$this->cmsUserVars->id.' = list.userid WHERE list.listid = '.intval($listid));
        }
    ...
    function replaceSubscriptionTag(&$allresults, $i, &$email)
    {
        ...
        if ($parameters->id == 'confirm') { //confirm your subscription link
            $myLink = acymailing_frontendLink('user&subid={subtag:subid}&task=confirm&key={subtag:key|urlencode}'.$lang);
            ...
            $myLink = acymailing_frontendLink('user&subid={subtag:subid}&task=modify&key={subtag:key|urlencode}'.$lang);
            if (empty($allresults[2][$i])) {
                return $myLink;
            }
            ...
            $myLink = acymailing_frontendLink('sub&task=optin&hiddenlists='.implode(',', $lists).'&user[email]={subtag:email|urlencode}'.$lang.$captchaKey);
            if (empty($allresults[2][$i])) {
                return $myLink;
            }
        ...
        $myLink = acymailing_frontendLink('user&subid={subtag:subid}&task=unsub&mailid='.$email->mailid.'&key={subtag:key|urlencode}'.$lang.'&'.acymailing_noTemplate());
